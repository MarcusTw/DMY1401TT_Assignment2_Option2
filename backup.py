...
def run_detector_and_save(detector, path, filename):
    img = load_img(path)
    converted_img  = tf.image.convert_image_dtype(img, tf.float32)[tf.newaxis, ...]
    start_time = time.time()
    result = detector(converted_img)
    ...
    img.save(os.path.join(app.config['CLASSIFICATION_FOLDER'], filename))
...
def classify(filename):
    ...
    # start_time = time.time()
    # run_detector_and_save(detector, image_path, filename)
    # end_time = time.time()
    # print("Time = ", end_time-start_time)
    time.sleep(10)
    
    return render_template('index.html', classified=filename)
